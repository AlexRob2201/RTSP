...
class Timer:
    ...
    def finished(self):
        return (dt.now() - self.start_time).total_seconds() >= self.interval
...
class RTSPMonitor(QMainWindow):
    def __init__(self):
        ...
        self.frame_queue = {}  # Черга для передачі кадрів між потоками
        self.device_streams = {}
    ...
    def start_monitoring(self):
        if self.current_device:
            ...
            def start_streaming_thread():
                self.frame_queue[self.current_device.id] = Queue()  # Створити чергу для поточного пристрою
                self.frame_queue_locks[self.current_device.id] = threading.Lock()  # Створити блокування для черги
                self.last_save_time[self.current_device.id] = dt.now()  # Зберегти час старту
                self.start_streaming(self.current_device.id, self.current_device.rtsp_url)
            
            self.streaming_thread = threading.Thread(target=start_streaming_thread)
            self.streaming_thread.start()

        self.refresh_device_list()

    def start_streaming(self, device_id, rtsp_url):
        def stream_thread():
            try:
                cap = cv2.VideoCapture(rtsp_url, cv2.CAP_FFMPEG)
                while cap.isOpened() and self.current_device and self.current_device.active:
                    ret, frame = cap.read()
                    if ret:
                        if device_id in self.frame_queue:
                            self.frame_queue[device_id].put(frame)
                            self.save_frames(device_id)  # Збереження кадрів
                            self.last_save_time[device_id] = dt.now()  # Оновити час останнього збереження
                    else:
        ...
        self.device_streams[device_id] = threading.Thread(target=stream_thread)
        self.device_streams[device_id].start()
    ...
    def save_frames(self, device_id):
        current_time = dt.now()
        if device_id in self.last_save_time and (current_time - self.last_save_time[device_id]).total_seconds() >= self.current_device.interval:
            self.last_save_time[device_id] = current_time  # Оновити час останнього збереження
            if self.frame_queue[device_id].qsize() > 0:
                frame = self.frame_queue[device_id].get()
                try:
                    self.save_frame(frame, self.current_device.name, self.current_device.save_path)
                except Exception as e:
                    print(f"Error while saving frame: {e}")

    def save_frame(self, frame, device_name, save_path):
        try:
